...
tone_keywords = {
    "Informational": [
        ...
        "presentation", "overview", "schedule", "invite"
    ],
    "Contentious": [
    ...
    "Empathetic": [
        ...
        "mental health", "kindness", "uplift", "hope"
    ],
    "Urgent": [
    ...
    "Persuasive": [
        "must", "should", "need to", "together we can", "make a difference",
        "join the movement", "believe", "hope", "inspire", "commit",
        "empower", "encourage", "motivate", "drive change"
    ],
    "Neutral": [
    ...
    "Critical": [
        "failure", "issue", "problem", "concern", "criticize", "unmet",
        "ineffective", "unjust", "lacking", "fall short", "flaw", "gap",
        "deficiency", "shortcoming"
    ],
    "Optimistic": [
        "progress", "future", "opportunity", "growth", "success", "hope",
        "achieve", "improve", "bright", "moving forward", "positive change",
        "innovation", "milestone", "celebrate"
    ]
}
    ...
    for cat, keywords in category_keywords.items():
        for kw in keywords:
            if kw in description:
                matched.append(cat)
                break
    return ", ".join(sorted(set(matched))) if matched else "Economy"

def tag_tones(description):
    description = str(description).lower()
    matched = []
...
df["tone"] = df["description"].apply(tag_tones)


# EXPORT FINAL CSV
# ==========================
df.to_csv("events_tagged.csv", index=False)
print("✅ Done tagging categories + tones!")
