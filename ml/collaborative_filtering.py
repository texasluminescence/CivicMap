...
def is_cold_start_user(user_id: str, threshold: int = 3) -> bool:
    ...
    The hybrid recommender uses this to decide whether to blend in
...
if __name__ == "__main__":
    ...
    if ids_civic1 and ids_civic2:
        ...
        status = "PASS" if overlap >= 0.60 else "LOW — check seed data"
        print(f"  Result           : {status}")
    else:
        print("  No recommendations returned — check user IDs in DB.")
    ...
    if ids_civic1 and ids_arts1:
        ...
        status2 = "PASS" if overlap2 <= 0.30 else "HIGH — interests may be too similar in seed data"
    ...
    try:
        ...
        print(f"  Returned {len(recs_new)} recommendations — model did not crash. PASS")
        if recs_new:
